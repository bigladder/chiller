...
class EnergyPlusEIR(ChillerModel):
    ...
    def set_system(self, system):
        ...
        # set kwarg variables
        self.capacity_temperature_coefficients = self.system.kwargs[
            "capacity_temperature_coefficients"
        ]
        self.eir_temperature_coefficients = self.system.kwargs[
            "eir_temperature_coefficients"
        ]
        self.eir_part_load_ratio_coefficients = self.system.kwargs[
            "eir_part_load_ratio_coefficients"
        ]
        ...
        if self.system.rated_net_condenser_capacity is None:
            energy_balance = self.system.rated_net_evaporator_capacity * (
                1.0 / self.system.rated_cop + 1.0
            )
            self.system.rated_net_condenser_capacity = energy_balance * (
                1.0 - self.loss_fraction_sum
            )

        self.system.metadata.has_hot_gas_bypass_installed = (
