...
class ASHRAE90_1(EnergyPlusEIR):
    ...
    def __init__(self):
        super().__init__()
        self.required_kwargs += ["path_type", "condenser_type", "compressor_type"]
    ...
    def set_system(self, system):
        ...
        system.kwargs["eir_temperature_coefficients"] = (
            self.curve_set.eir_temperature_coefficients
        )
        system.kwargs["eir_part_load_ratio_coefficients"] = (
            self.curve_set.eir_part_load_ratio_coefficients
        )
        system.kwargs["capacity_temperature_coefficients"] = (
            self.curve_set.capacity_temperature_coefficients
        )
        ...
        if self.maximum_scaled_rated_capacity == float("inf"):
            size_description = (
                f"{to_u(self.minimum_scaled_rated_capacity,'ton_ref'):.1f}+"
            )
        else:
        ...
        system.metadata.uuid_seed = sha256(
    ...
    def __hash__(self):
        return hash(
            (
                ...
                self.system.rated_net_evaporator_capacity,
                self.system.rated_cop,
            )
        )
